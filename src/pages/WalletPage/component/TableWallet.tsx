...
function Row(props: { row: ReturnType<typeof createData> }) {
  ...
  const columnsDeposite = [
    {
      ...
      flex: 1,
      renderCell: (params: any) => <Box>{`NAP${params.value}`}</Box>,
    },
    {
      field: "create_Date",
  ...
  const columnsWithdrawal = [
    {
      ...
      headerName: t("wallet.title_transaction_code"),
      flex: 1,
      renderCell: (params: any) => <Box>{`RUT${params.value}`}</Box>,
    },
    {
      field: "create_Date",
    ...
    {
      field: "dateApprove",
      headerName: t("wallet.title_approval_date"),
      flex: 1,
      renderCell: (params: any) => (
        <Box>
    ...
    {
      field: "status",
      headerName: t("wallet.title_status"),
      flex: 1,
      renderCell: (params: any) =>
  ...
  const columnsBooking = [
    {
      field: "address",
      headerName: t("wallet.title_address_get_motorbike"),
      flex: 1,
    ...
    {
      field: "status",
      headerName: t("wallet.title_status_booking"),
      flex: 1,
      renderCell: (params: any) =>
...
export default function CollapsibleTable({
    ...
    try {
      ...
      BookingService.getListBookingCurrentUser().then((data) => {
        const bookingsThisMonthYear = data.filter((booking) => {
          ...
          return (
            bookingMonth === selectedDate.month() + 1 &&
            bookingYear === selectedDate.year()
          );
        });
  ...
  const totalWithdrawal = withdrawals.reduce(
    (total, it) => total + (it.withdraw || 0),
    0
  );
  const rows = [
    createData(
      t("wallet.title_deposit_transaction"),
      deposites.length,
      totalDeposits,
      "NAP",
      deposites,
    ...
    createData(
      t("wallet.title_withdrawal_transaction"),
      withdrawals.length,
      totalWithdrawal,
      "RUT",
      withdrawals,
    ...
    createData(
      t("wallet.title_history_booking"),
      bookings.length,
      -1,
      "BOOK",
      [],
