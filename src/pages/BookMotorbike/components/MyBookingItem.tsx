...
import { CalendarTodayOutlined, CheckCircleOutline, ErrorOutline, KeyboardArrowDown, KeyboardArrowUp, LocationOnOutlined, WarningAmber } from "@mui/icons-material";
            ...
            case BookingStatus.PendingDelivery:
                return (
                    <Chip
                        ...
                        color="warning"
                        icon={<WarningAmber />}
                        label={t('myBooking.bookingStatus.pendingDeliveryGuest')}
                    />
                );
            case BookingStatus.Delivered:
                return (
                    <Chip
                        ...
                        color="success"
                        icon={<CheckCircleOutline />}
                        label={t('myBooking.bookingStatus.deliveredGuest')}
                    />
                );
                        ...
                        <Box display={"flex"} flexDirection={"column"} gap={"16px"} p={3}>
                            ...
                            <Box display={"flex"} flexDirection={"row"} justifyContent={"space-between"} alignItems={"center"}>
                                ...
                                <Box display={'flex'} flexDirection={'row'} alignItems={'start'} justifyContent={'center'} sx={{ gap: '4px' }}>
                                    <MyCustomButton
                                        ...
                                        onClick={() => { navigate(`/${ROUTES.booking.detail}/${booking.bookingId}`) }}
                                        width="auto"
                                        height="36px"
                                        fontSize={16}
                                        fontWeight={400}
                            ...
                            >
                                {booking.motorbikes.map((motorbike: Motorbike, index: number) => (
                                    <MotorbikeInforCard key={index} motorbike={motorbike} isIntroduced={true} isFavoritePage={false} startDate={dayjs(booking.startDatetime).format("DD/MM/YYYY HH:mm")} endDate={dayjs(booking.endDatetime).format("DD/MM/YYYY HH:mm")} searchedAddress={booking.address} />
                                ))}
                            </AnimatedBox>
                        </Box>
                    </Paper>)))}
