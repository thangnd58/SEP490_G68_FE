...
import { CartInforResponse, Motorbike, MotorbikeCart, ShoppingCart } from "../../utils/type";
...
function CartPage() {
    ...
    const [motorbikeCart, setMotorbikeCart] = useState<CartInforResponse[]>([]);
    const [open, setOpen] = useState(Array(motorbikeCart.length).fill(false));
            ...
            {/* Check giỏ hàng trống */}
            {
                isLoad ? (<Box width={"100%"} display={'flex'} flexDirection={'column'} alignItems={'center'} justifyContent={'center'} sx={{ gap: '8px' }}>
                    ...
                    motorbikeCart.length === 0 ? (
                        ...
                        motorbikeCart.map((item: CartInforResponse, index: number) => (
