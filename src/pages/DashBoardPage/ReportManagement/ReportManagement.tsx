...
const ReportManagement = () => {
  ...
  const columns = [
    {
      field: "reporter.name",
      headerName: t("report.reporter"),
      width: 150,
      valueGetter: ({ row }: any) => row.reporter.name,
    },
    {
    ...
    },
    {
      ...
      width: 150,
      renderCell: (params: any) => (
        <Box>{params.row.responder ? params.row.responder.name : "N/A"}</Box>
      ),
    },
    {
    ...
    },
    {
      ...
      renderCell: (params: any) => (
        <Box sx={{ cursor: "pointer" }} display={"flex"}>
          <MyIcon
            icon={
              params.row.status === ReportStatus.Responded ? (
                <Visibility />
              ) : (
                <EditIcon />
              )
            ...
            onClick={() =>
              navigate(`${ROUTES.admin.manageReport}/${params.value}`)
            }
          />
        </Box>
      ),
