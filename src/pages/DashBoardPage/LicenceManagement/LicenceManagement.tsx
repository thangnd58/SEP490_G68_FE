...
const LicenceManagement = () => {
  ...
  const columns = [
    {
      field: "licenceNumber",
      headerName: t("dashBoardManager.licenseManager.columnLicenseNumber"),
      width: 150,
    },
    {
      field: "fullName",
      headerName: t("dashBoardManager.licenseManager.columnFullName"),
      width: 200,
    },
    {
      field: "dob",
      headerName: t("dashBoardManager.licenseManager.columnDateOfBirth"),
      width: 150,
    },
    {
    ...
    },
    {
      ...
      renderCell: (params: any) => (
        <Box sx={{ cursor: "pointer" }}>
          <MyIcon
            icon={params.row.status === 1 ? <Visibility /> : <EditIcon />}
            position="right"
            hasTooltip
            tooltipText={t("userProfile.BtnChange")}
            onClick={() =>
              navigate(`${ROUTES.admin.licenceregister}/${params.value}`)
            }
          />
        </Box>
      ),
