...
const LayoutAdmin = ({ children }: LayoutAdminProps) => {
    ...
    return (
        <Box display="flex" flexDirection="column">
            ...
            {/* Background Part */}
            <Box position="sticky" height={"107px"} top={"85px"} zIndex={998} width={"100%"} sx={{ backgroundColor: "#8B4513" }} />

            {/* Main Part */}
            <Box component="main" display="flex" >
                {/* Side bar */}
                <Box component="aside" width={"300px"} height={"600px"} position="sticky" top={"85px"} style={{ zIndex: 999 }} sx={{ marginTop: '-107px', mb: "24px" }} >
                    <Box sx={{ backgroundColor: "#8B4513" }} display={'flex'} justifyContent={'start'} alignItems={'center'} padding={"16px"} gap={"8px"}>
                        <Avatar sx={{ width: 75, height: 75 }} src={user?.avatarUrl} alt={user?.name} />
                    ...
                    <Box sx={{ border: "1px solid #E0E0E0", margin:"16px" }}>
                        <NavigationAdmin />
                    </Box>
                </Box>

                {/* Content */}
                <Box component="section" p={"24px 32px 32px 0px"} style={{ overflow: 'auto', zIndex: 999, marginTop: '-107px' }} >
                    {children}
                </Box>
            </Box>
        </Box>
